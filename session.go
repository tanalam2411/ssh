...
type Session interface {
	...
	// PublicKey returns the PublicKey used to authenticate.
	// If a public key was not used it will return nil.
	PublicKey() PublicKey

	// Context returns the connection's context. The returned context is always non-nil
	// and holds the same data as the context passed into auth handlers and callbacks.
	//
	// The context is canceled when the client's connection closes or I/O operation fails.
	Context() context.Context

	// Permissions returns a copy of the Permissions object that was available for setup
	// in the auth handlers via the Context.
	...
	// The channel must handle signal sends or it will block the SSH request loop.
	// Registering nil will unregister the channel from signal sends.
	// During the time no channel is registered signals are buffered up to a reasonable amount.
...
type session struct {
	...
	exited    bool
	pty       *Pty
	winch     chan Window // winch -> window channel
	env       []string
	ptyCb     PtyCallback
...
func (sess *session) Write(p []byte) (n int, err error) {
	if sess.pty != nil {
		...
		// this is hardcoded shortcut since we don't support terminal modes.
